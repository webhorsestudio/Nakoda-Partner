-- Migration: Create Call Logs Table for Acefone Call Masking (Corrected)
...
-- Create RLS policies (simplified to avoid auth.uid() casting issues)
CREATE POLICY "Allow all operations for authenticated users" ON public.call_logs
    FOR ALL USING (true);
