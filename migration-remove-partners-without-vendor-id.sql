-- Migration: Remove partners without vendor_id
-- Description: Soft deletes all partners who don't have a vendor_id value
-- Date: 2025-01-27
...
-- Step 4: Soft delete partners without vendor_id (set deleted_at timestamp)
UPDATE partners 
SET 
  deleted_at = NOW(),
  updated_at = NOW(),
  notes = COALESCE(notes, '') || ' [DELETED: No vendor_id assigned]'
WHERE (vendor_id IS NULL OR vendor_id = '') 
...
-- Step 6: Verify no active partners remain without vendor_id
SELECT 
  'VERIFICATION: Active partners without vendor_id' as verification_type,
  COUNT(*) as count
FROM partners 
